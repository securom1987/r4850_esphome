...
CONF_INPUT_VOLTAGE = "input_voltage"                                            # verwendete Variablen - Namen
...
TYPES = [                                                                       # Array mit den oben definierten Namen erstellen
    CONF_INPUT_VOLTAGE,
...
CONF_POWER_DEMAND = "power_demand"                                                  # hinzugefügt
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_POWER_DEMAND): sensor.sensor_schema(               # hinzugefügt
            unit_of_measurement=UNIT_WATT,
            icon=ICON_EMPTY,
            accuracy_decimals=0,
            device_class=DEVICE_CLASS_POWER,
...
async def to_code(config):
    ...
    if CONF_POWER_DEMAND in config:                                         # hinzugefügt
